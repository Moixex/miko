...
export class Bot extends Client {
  ...
  public async start() {
    ...
    if (!osuAPIKey)
      throw new Error("No osu! API key provided (process.env.OSU_API_KEY).");
    ...
    Logger.out({
      prefix: "[CLIENT]",
      message: `Registering events...`,
      color: "Purple",
      important: true,
    });
    await EventHandler.registerEvents(this); // ✅ Pasa el client
    Logger.out({
      prefix: "[CLIENT]",
      message: `Successfully registered ${this.events.size} event(s).`,
      color: "Purple",
    });
    ...
    await CommandHandler.registerCommands(this); // ✅ Pasa el client
    Logger.out({
      prefix: "[CLIENT]",
      message: `Successfully registered ${this.commands.size} slash command(s) and ${this.dev_commands.size} developer command(s).`,
      color: "Purple",
    });
